...
async def move_player(player_id: uuid.UUID, new_position: Square, controller: ControllerDep):
    try:
...
@router.get("/events")
async def game_events(request: fastapi.Request, controller: ControllerDep):

...
def add_route_type(route_type_data: RouteTypeCreate, controller: ControllerDep):
    try:
        controller.add_route_type(route_type_data.route_type, "")
    except RuntimeError as e:
        raise HTTPException(status_code=400, detail=str(e))
    return "Route type added successfully."


@router.delete("/types/{route_type}", summary="Remove a route type")

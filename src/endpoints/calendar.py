...
async def add_day(params: DayCreate, controller: ControllerDep):
    try:
        await controller.add_day(params.day_type)
        await controller.get_event_manager().broadcast_event("game.calendar.day.added", {"type": params.day_type})
...
async def modify_day(day: DayUpdate, controller: ControllerDep):
    try:    
        await controller.modify_day(day.day_number, day.day_type)
        await controller.get_event_manager().broadcast_event("game.calendar.day.modified", {"number": day.day_number, "type": day.day_type})
...
async def delete_day(day_number: int, controller: ControllerDep):
    try:
        ...
        await controller.get_event_manager().broadcast_event("game.calendar.day.removed", {"number": day_number})
    except IndexError as e:
        raise HTTPException(400, str(e))
